...
export function useBlogBySlug(slug: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<{ blog: Blog }>(`/blog/${slug}`).then((r) => r.data),
    enabled: !!slug,
  });
}
...
export function useBlog(id: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<{ blog: Blog }>(`/blog/${id}`).then((r) => r.data.blog),
    enabled: !!id,
  });
}
...
export function useCreateBlog() {
  ...
  return useMutation({
    ...
    onSuccess: (blog) => {
      ...
      qc.setQueryData(queryKeys.blogs.detail(blog.slug), { blog });
...
export function useUpdateBlog() {
  ...
  return useMutation({
    ...
    onSuccess: (blog) => {
      // Refresh all blog lists and the specific detail cache
      qc.invalidateQueries({ queryKey: queryKeys.blogs.lists() });
      qc.invalidateQueries({ queryKey: queryKeys.blogs.mine() });
      qc.setQueryData(queryKeys.blogs.detail(blog.slug), { blog });
  ...
  return useMutation({
    ...
    onSuccess: (_, id) => {
      ...
      qc.setQueriesData<{ blogs: Blog[] }>(
        ...
        (old) =>
          old
            ? { ...old, blogs: old.blogs.filter((b) => b._id !== id) }
            : old
      );
      // Remove own-blog list entry too
...
export function useLikeBlog() {
  ...
  return useMutation<ApiResponse<LikeResponse>, Error, string>({
    ...
    onMutate: async (id: string) => {
      // Cancel any in-flight refetches for blog detail/list
