...
export default function ArticleDetailPage() {
  ...
  const submitReply = async (parentId: string) => {
    ...
    await postComment.mutateAsync({ articleId: article!._id, body: replyText, parent: parentId });
    setReplyTo(null);
    setReplyText("");
  };
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1">
          <EmptyState icon="📄" title="Article not found" description="This story may have been removed." />
        </main>
        <Footer />
      </div>
    );
            ...
            <article>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-6 mb-6 border-b border-[var(--color-border)]">
                <div className="flex items-center gap-3">
                  <Link href={`/writers/${article.author._id}`}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-ink-500 font-sans">
                      {article.publishedAt && <span>{formatDate(article.publishedAt, "MMMM dd, yyyy")}</span>}
                      <span>·</span>
              ...
              {article.tags && article.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-8 pt-8 border-t border-[var(--color-border)]">
                  <span className="text-xs font-sans font-semibold text-ink-500 uppercase tracking-widest">Tags:</span>
                  {article.tags.map(tag => (
                    <Link key={tag._id} href={`/news?search=${tag.name}`}
              ...
              <div className="flex items-center gap-3 mt-6 py-4 border-t border-b border-[var(--color-border)]">
                <button
                  ...
                  <Heart
                    size={15}
                    className={cn(article.isLiked ? "fill-ember-600 text-ember-600" : "")}
                  />
              ...
              <div className="mt-8 p-5 bg-ink-50 rounded-2xl border border-[var(--color-border)]">
                <div className="flex items-center justify-between">
                  <div className="flex items-start gap-4">
                    <Link href={`/writers/${article.author._id}`} className="shrink-0">
                      {article.author.avatar?.url ? (
                        <img src={article.author.avatar.url} alt={article.author.name}
                    ...
                    <div>
                      <p className="text-[11px] font-sans font-bold uppercase tracking-widest text-ink-400 mb-1">Written by</p>
                      <Link href={`/writers/${article.author._id}`}
                        className="font-display font-bold text-lg text-ink-900 hover:text-ember-600 transition-colors">
                        {article.author.name}
                      </Link>
                      {article.author.bio && (
                        <p className="text-sm text-ink-600 font-body mt-1">{article.author.bio}</p>
                      )}
                    </div>
                  </div>
                  <div>
            ...
            <aside>
              <div className="sticky top-20 space-y-5">
                ...
                <div className="border border-[var(--color-border)] rounded-xl p-4 bg-white">
                  <p className="text-[11px] font-sans font-bold uppercase tracking-widest text-ink-400 mb-3">About the Writer</p>
                  <div className="flex items-center gap-3">
                    <Link href={`/writers/${article.author._id}`} className="shrink-0">
                      {article.author.avatar?.url ? (
                        <img src={article.author.avatar.url} alt={article.author.name}
                        ...
                        <div className="w-12 h-12 rounded-full bg-ember-600 flex items-center justify-center text-white font-bold">
                          {getInitials(article.author.name)}
                        </div>
                      )}
                    </Link>
                    <div>
                      ...
                      {article.author.bio && (
                        <p className="text-xs text-ink-600 font-body mt-1 line-clamp-3">{article.author.bio}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {/* Related */}
                <RelatedArticles categorySlug={article.category.slug} currentId={article._id} />
              </div>
            </aside>
          </div>
        </div>
...
function CommentThread({
  comment, replyTo, replyText, isLoggedIn,
  onSetReply, onReplyTextChange, onSubmitReply, isPending,
}: {
  comment: Comment;
  replyTo: string | null;
  replyText: string;
                ...
                <div className="flex-1">
                  <div className="bg-ink-50 border border-[var(--color-border)] rounded-xl px-3 py-2.5">
                    ...
                    <p className="text-ink-700 font-body text-xs leading-relaxed">
                      {reply.isDeleted ? <em className="text-ink-400">[deleted]</em> : reply.body}
                    </p>
                  </div>
                </div>
              </div>
...
function RelatedArticles({ categorySlug, currentId }: { categorySlug: string; currentId: string }) {
  const { data } = useArticles({ category: categorySlug, limit: 5, status: "published" });
  const related  = (data?.articles ?? []).filter(a => a._id !== currentId).slice(0, 4);
  if (!related.length) return null;
  return (
    <div className="border border-[var(--color-border)] rounded-xl overflow-hidden bg-white">
      <div className="px-4 py-3 border-b border-[var(--color-border)]">
        <p className="text-[11px] font-sans font-bold uppercase tracking-widest text-ink-400">Related Stories</p>
      </div>
      <div className="divide-y divide-[var(--color-border)]">
        {related.map(a => (
          <Link key={a._id} href={`/articles/${a.slug}`}
            ...
            {a.featuredImage?.url && (
              <img src={a.featuredImage.url} alt={a.title}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-ink-400 font-sans mt-1">{timeAgo(a.publishedAt)}</p>
            </div>
          </Link>
        ))}
      </div>
