...
import { useCreateBlog } from "@/hooks/useData";
...
export default function CreateBlogPage() {
  ...
  const createMut = useCreateBlog(); // from useData hook — see below for hook signature
  ...
  // Validation: title ≥ 5 chars, content (stripped) ≥ 50 chars
  const isValid =
    title.trim().length >= 5 &&
  ...
  const handlePublish = async () => {
    ...
    try {
      const blog = await createMut.mutateAsync(fd);
      // Navigate to the new blog's slug page
      router.push(`/blogs/${blog.slug}`);
    } catch {
      // Error handled inside the mutation / global error handler
    }
  };
    ...
    <div className="min-h-screen flex flex-col bg-(--color-bg)">
      ...
      <main className="flex-1">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          ...
          {/* ── Tip banner ── */}
          <div className="mb-5 px-3 py-2 bg-sky-50 border border-sky-200 rounded-xl flex items-center gap-2 text-sm font-sans text-sky-800">
            <Lightbulb size={14} className="shrink-0 text-sky-500" />
            Your blog will be published immediately and visible to everyone.
          </div>
            ...
            <div className="space-y-5">
              ...
              >
                {coverPreview ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 gap-2">
                    ...
                    <p className="font-sans font-semibold text-ink-700 text-sm">
                      Add a cover image
                    </p>
                    <p className="font-sans text-ink-400 text-xs">
              ...
              <div>
                <label className="block text-[11px] font-sans font-bold text-ink-500 uppercase tracking-widest mb-2">
                  ...
                  <span className="ml-2 normal-case font-normal text-ink-400">
                    (min 50 characters)
                  </span>
                </label>
                <TiptapEditor
