...
import { useBlogBySlug, useLikeBlog, useBlogComments, usePostComment } from "@/hooks/useData"; // ✅ useBlogBySlug
...
export default function BlogDetailPage() {
  ...
  const { data: blogData, isLoading } = useBlogBySlug(slug ?? "");
  const blog = blogData?.blog;

  
  const { data: commentData } = useBlogComments(blog?._id ?? "", 1);
        ...
        <main className="flex-1 max-w-3xl mx-auto px-6 py-12 w-full">
          ...
          <div className="space-y-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="skeleton h-5 rounded" style={{ width: `${75 + (i % 3) * 10}%` }} />
            ))}
          </div>
        </main>
        <Footer />
      ...
      <main className="flex-1">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          ...
          <article>
            ...
            {/* Author row */}
            <div className="flex items-center justify-between pb-6 mb-6 border-b border-(--color-border)">
              <div className="flex items-center gap-3">
                {blog.author ? (
                  ...
                  <div>
                    <p className="font-sans font-semibold text-ink-900 text-sm">Unknown Author</p>
                    <div className="flex items-center gap-2 text-xs text-ink-500 font-sans">
                      <span>{formatDate(blog.createdAt, "MMMM dd, yyyy")}</span>
                      <span>·</span>
                      <span className="flex items-center gap-1"><Clock size={11} /> {blog.readTime} min read</span>
            ...
            {blog.featuredImage?.url && (
              <div className="mb-8 rounded-2xl overflow-hidden">
                <img src={blog.featuredImage.url} alt={blog.title}
                  className="w-full object-cover max-h-115" />
              </div>
            )}
            ...
            {/* Actions */}
            <div className="flex items-center gap-3 mt-8 py-4 border-t border-b border-(--color-border)">
              <button onClick={handleLike}
                ...
                {/* ✅ isLiked comes from server via optimistic cache — no local liked state needed */}
                <Heart size={15} className={cn(blog.isLiked ? "fill-ember-600 text-ember-600" : "")} />
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1">
        ...
        {comment.replies && comment.replies.length > 0 && (
          <div className="mt-4 pl-4 border-l-2 border-(--color-border) space-y-3">
            {comment.replies.map(reply => (
              <div key={reply._id} className="flex gap-2">
                ...
                <div className="flex-1 bg-ink-50 border border-(--color-border) rounded-xl px-3 py-2">
                  ...
                  <p className="text-ink-700 font-body text-xs">
