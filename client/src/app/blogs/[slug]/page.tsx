          ...
          <article>
            ...
            {(blog.tags ?? []).length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-4">
                {blog.tags!.map((tag) => (
                  <span
                    key={tag}
                    className="text-[11px] font-sans font-medium text-ink-500 bg-ink-100 px-2.5 py-0.5 rounded-full"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <div className="flex items-center justify-between pb-6 mb-6 border-b border-(--color-border)">
              ...
              <div className="flex items-center gap-3 text-xs text-ink-500 font-sans">
                <span className="flex items-center gap-1">
                  <Eye size={13} /> {formatNumber(blog.views)}
                </span>
                <span className="flex items-center gap-1">
                  <Heart size={13} /> {formatNumber(blog.likes)}
                </span>
              </div>
            </div>
...
function BlogCommentThread({
  ...
  return (
    <div className="flex gap-3">
      {/* Avatar */}
      <div className="w-8 h-8 rounded-full bg-ink-200 flex items-center justify-center text-ink-700 text-xs font-bold shrink-0 overflow-hidden">
