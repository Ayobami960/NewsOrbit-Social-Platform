...
export default function ProfilePage() {
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data } = await authFetch<{ user: UserType }>("/users/me", {
        method: "PATCH",
        body: fd,
      });
      setUser(data.user);
  ...
  const inputCls = "w-full px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] text-ink-900 font-sans text-sm placeholder:text-ink-400 outline-none focus:border-ember-600 focus:ring-2 focus:ring-ember-600/20 transition-all";
              ...
              <div className="bg-white border border-[var(--color-border)] rounded-2xl p-6">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { label: "Articles",     value: String(user.stats?.totalArticles ?? 0) },
