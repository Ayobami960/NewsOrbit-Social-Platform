...
const ROLE_BADGE_CLASS: Record<string, string> = {
  ...
  writer:  "bg-emerald-500/10 border-emerald-500/20 text-emerald-400",
  manager: "bg-violet-500/10 border-violet-500/20 text-violet-400",
};
...
// ─────────────────────────────────────────────────────────────────────────────
// Manager scope badge shown on the confirm_email step
// ─────────────────────────────────────────────────────────────────────────────

function ManagerScopeBadge({ scope }: { scope: string }) {
...
export default function AcceptInvite() {
  ...
  const handleSetPassword = async () => {
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : "Failed to complete setup."
      );
    } finally {
      setLoading(false);
    }
  ...
  // ── Derived ───────────────────────────────────────────────────────────────
  const roleLabel     = tokenInfo ? (ROLE_LABEL[tokenInfo.role] ?? tokenInfo.role) : "";
  const badgeClass    = tokenInfo ? (ROLE_BADGE_CLASS[tokenInfo.role] ?? "bg-zinc-800 border-zinc-700 text-zinc-400") : "";
  const dotClass      = tokenInfo ? (ROLE_DOT_CLASS[tokenInfo.role]  ?? "bg-zinc-500") : "";
  const isManager     = tokenInfo?.role === "manager";
      ...
      <div className="relative w-full max-w-sm">
        ...
        {step === "confirm_email" && tokenInfo && (
          <div>
            ...
            <div className="mb-6">
              <span
                className={`inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest mb-3 px-2.5 py-1 rounded-full border ${badgeClass}`}
              >
                <span className={`w-1.5 h-1.5 rounded-full inline-block ${dotClass}`} />
                You're invited as {roleLabel}
              </span>

              <h1 className="text-zinc-100 text-2xl font-semibold leading-snug mb-2 mt-3">
                Welcome, {tokenInfo.name.split(" ")[0]}
              </h1>
              <p className="text-zinc-400 text-sm leading-relaxed">
                We'll send a 6-digit verification code to confirm you own
                this address.
              </p>

              {/* Manager scope pill */}
              {isManager && tokenInfo.inviteManagement && (
                <ManagerScopeBadge scope={tokenInfo.inviteManagement} />
              )}
            </div>
          ...
          <div>
            ...
            <div className="mb-7">
              ...
              <p className="text-zinc-400 text-sm leading-relaxed">
                ...
                <span className={`font-medium px-1.5 py-0.5 rounded text-xs border ${badgeClass}`}>
                  {roleLabel}
                </span>{" "}
                account.
              </p>
        ...
        {step === "done" && tokenInfo && (
          <div className="text-center py-6">
            ...
            >
              <svg
                className={`w-8 h-8 ${isManager ? "text-violet-400" : "text-emerald-400"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <p className="text-zinc-400 text-sm leading-relaxed mb-1">
              Your{" "}
              <span
                className={`font-medium px-1.5 py-0.5 rounded text-xs border ${badgeClass}`}
              >
                {roleLabel}
              </span>{" "}
              account is ready.
            ...
            {/* Show scope on done screen for managers */}
            {isManager && tokenInfo.inviteManagement && (
              <p className="text-zinc-600 text-xs mt-1">
                Scope:{" "}
                <span className="text-violet-400">
                  {tokenInfo.inviteManagement.replace(/_/g, " ")}
                </span>
              </p>
            )}
            ...
            <div className="mt-6 h-0.5 bg-zinc-800 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full animate-[progress_2.5s_linear_forwards] ${
                  isManager ? "bg-violet-500" : "bg-emerald-500"
                }`}
              />
            </div>
          </div>
