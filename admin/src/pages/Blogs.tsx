 ...
 * Blogs go live immediately — no moderation.
 * Admins can delete policy-violating content.
 */
    ...
    <Layout title="Community Blogs">
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 backdrop-blur-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="py-20">
            <Empty message="No community blogs yet" />
          </div>
        ) : (
          <>
            {/* ── DESKTOP TABLE (md+) ──────────────────── */}
                ...
                <thead>
                  <tr className="border-b border-zinc-800/60">
                    {["Blog Post", "Author", "Tags", "Engagement", "Posted", ""].map(h => (
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-500 uppercase tracking-widest"
                      >
                        {h}
                      </th>
                    ))}
                        ...
                        <div className="flex items-start gap-3">
                          {b.featuredImage?.url ? (
                            <img
                              ...
                              alt=""
                              className="shrink-0 w-14 h-14 object-cover rounded-lg border border-zinc-700/60"
                            />
                          ) : (
                          ...
                          <div className="min-w-0">
                            <p className="text-zinc-100 font-semibold text-sm leading-snug mb-1 line-clamp-2">
                              {b.title}
                            </p>
                            {b.excerpt && (
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-1 max-w-30">
                          {(b.tags ?? []).slice(0, 3).map(t => (
                            <span
                              key={t}
                              className="px-2 py-0.5 rounded-full bg-zinc-800 border border-zinc-700/60 text-zinc-400 text-[10px] font-medium"
                            >
                              #{t}
                            </span>
                          ))}
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="flex items-center gap-1 text-[11px] text-zinc-400">
                              <Heart size={11} className="text-rose-500/70" />
                              {b.likes}
                            </span>
                            <span className="flex items-center gap-1 text-[11px] text-zinc-400">
                              <MessageCircle size={11} className="text-blue-500/70" />
                  ...
                  <div className="flex gap-3">
                    ...
                    {b.featuredImage?.url ? (
                      <img
                        src={b.featuredImage.url}
                        alt=""
                        className="shrink-0 w-16 h-16 object-cover rounded-xl border border-zinc-700/60"
                      />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-zinc-100 font-semibold text-sm leading-snug line-clamp-2 mb-1">
                        {b.title}
                      </p>
                      {/* Author row */}
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        {(b.tags ?? []).slice(0, 3).map(t => (
                          <span
                            key={t}
                            className="px-1.5 py-0.5 rounded-full bg-zinc-800 border border-zinc-700/40 text-zinc-500 text-[10px] font-medium"
                          >
                            #{t}
                          </span>
                        ))}
