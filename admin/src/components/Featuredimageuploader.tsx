...
// // Re-export so ArticleEditor can import from here
// export type { UploadedImage };

// interface Props {
//   value: string;                                    // current saved image URL ("" = none)
//  onChange: (image: UploadedImage | null, file?: File) => void;
//   folder?: string;
// }
...
//   // Show live blob preview while uploading; fall back to the saved URL from props
...
//   // ── Render: image set (saved URL or uploading blob preview) ─────────────
//   if (displaySrc) {
//     return (
...
//   // ✅ Keep a ref to the raw File the user picked so we can pass it to onChange
//   const pendingFileRef = useRef<File | null>(null);

//   const { preview, uploading, error, upload, remove } = useImageUpload({
//     folder,
//     maxSizeMB: 10,
//     // ✅ onSuccess now receives the UploadedImage AND we still have the File in the ref
//     onSuccess: (image) => {
//       onChange(image, pendingFileRef.current ?? undefined);
...
//     pendingFileRef.current = file;   // ✅ stash the File before kicking off the upload
//     upload(file);
//   }, [upload]);
...
//   // ── Hidden file input (shared between both render paths) ────────────────
//   const fileInput = (
//     <input
//       ref={inputRef}
//       type="file"
...
//           {/* Replace / Remove on hover (hidden while uploading) */}
//           {!uploading && (
//             <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
//               <button
//                 type="button"
